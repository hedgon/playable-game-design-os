...
TECH('platform-requirements',[
  ...
  {n:'Requirement checklist mapped to platform documents', how:`Turn each platform's public requirements page, or its NDA outline, into a checklist item with an owner and the build it is verified in.`, fit:`Any multi-platform project heading toward its first certification pass.`, cost:`Upkeep: the checklist has to track the platform's own document version and revalidate after every revision.`, alt:`Rely on the certification pass itself to surface gaps, which is slower and costs more per finding.`}
]);
...
INTERVIEW('platform-requirements',{
  junior:[
    { q:`Why does a game need to handle "suspend and resume" as its own case?`,
      ...
      follow:`How would you test that your game actually handles a suspend, not just a menu pause?`,
...
T('certification-and-review',{ d:'platforms', t:'Certification and store review', tag:'Every platform has a gate, a clock and a list of reasons it says no. Plan the calendar around the clock, not around hope.',
  ...
  think:{ q:[`Have we scheduled the review window as a fixed date on the calendar, with buffer for at least one resubmission?`,`Does the store page describe only what ships at launch, with nothing promised that the build does not do?`,`Have we run the platform's own validator or pre-check before the real submission?`,`Is a content or privacy disclosure form (Steam's Content Survey, Google's Data safety form, Apple's privacy labels) filled in and accurate?`,`What is the resubmission cost if this specific build fails, and can the schedule absorb it?`],
    trade:[`Submitting early with buffer costs calendar time up front and protects the launch date; submitting late to maximize content risks the date entirely.`,`An expedited or Fastlane review where the platform offers one (Apple expedited review, Xbox Fastlane), spent only on a genuine emergency, not routine lateness.`],
  ...
  prompts:[{l:'Review calendar',p:`Our target launch date is [DATE] and we are submitting to [PLATFORMS]. Using each platform's published review turnaround (or noting where it is confidential), build a submission calendar working backward from launch, with buffer for one resubmission per platform.`},
    {l:'Pre-submission check',p:`Here is our store page draft and a summary of what the current build actually does: [PASTE]. List every claim on the page the build does not yet deliver, and check for the common rejection causes: crashes, placeholder content, and missing content or privacy disclosure forms.`}],
...
INTERVIEW('certification-and-review',{
  ...
  mid:[
    ...
    { q:`A store page was written before the build was finished. What do you check before submission?`,
      a:`Every claim on the page against what actually ships at launch, because a page promising a feature the build does not have is a known, avoidable rejection cause on multiple platforms. Update the page the moment scope changes rather than leaving it to catch up at submission time.`,
  ...
  senior:[
    ...
    { q:`A team keeps missing launch dates because certification "always takes longer than planned." What is actually wrong?`,
...
T('ratings-and-disclosures',{ d:'platforms', t:'Ratings, privacy forms and disclosures', tag:'The rating board and the privacy form are part of the submission, and a wrong answer blocks the release as hard as a crash does.',
  ...
  think:{ q:[`Which of our storefronts share IARC, and which run their own questionnaire?`,`Does a Japan release or a disc SKU exist in our plan, and does it have its own rating line item?`,`What does every SDK in the build collect, not just the code we wrote?`,`If the game has any paid random item, where does the odds disclosure have to appear?`],
    ...
    traps:[`Assuming one IARC answer covers Steam or Apple.`,`Treating Japan as "the same as everywhere else with a different language".`,`Declaring "no data collected" without checking the ad or analytics SDK.`,`Discovering a paid-random-item mechanic that needed disclosure only after a reviewer flags it.`],
  ...
  ai:{ yes:[`Draft the privacy-form answers from a list of the SDKs actually in the build.`,`Draft odds-disclosure copy from the drop table's real values.`,`Summarize a rating board's public content descriptors against your game's content.`],
  ...
  verify:[`Does the privacy or Data safety form match every SDK actually shipping in this build?`,`Does the odds-disclosure text match the exact drop table shipped, not an earlier tuning pass?`,`Is there a rating line item for Japan or a physical SKU if either exists in the plan?`],
...
INTERVIEW('ratings-and-disclosures',{
  ...
  mid:[
    { q:`A store's privacy form asks what the game collects. Your code collects nothing, but you shipped an ad SDK. What do you answer?`,
      a:`You answer based on what the ad SDK actually collects, not your own code, because these forms explicitly cover third-party SDK data. You pull the SDK vendor's own data-collection disclosure rather than guessing, and you redo this whenever the SDK version changes.`,
    ...
    { q:`The game has a gacha-style paid pull. Where does the odds disclosure need to appear, and what happens if it does not?`,
      ...
      red:`Treats odds disclosure as optional flavor text rather than a submission requirement.` }
  ],
  senior:[
...
T('store-presence',{ d:'platforms', t:'Store pages, pricing and getting paid', tag:'The store page sells the game before the game does, and the pricing and revenue terms decide what that sale is worth.',
  what:`Everything that turns a finished build into a listing someone can buy: the required capsule and screenshot assets, a Coming Soon page that opens wishlisting before launch, pre-orders or pre-registration campaigns, base and regional pricing with each store's discount rules, the store's cut of revenue, and the payout and tax setup behind the money actually arriving. Revenue share is not one number: Steam is reported to use tiers that fall as lifetime revenue rises, Apple splits standard sales from qualifying subscriptions and its Small Business Program, Google's fee is mid-transition and differs by region, Microsoft Store on PC is a flat rate, Epic gives the first slice of revenue back entirely before splitting the rest, and most console shares are not published at all.`,
  ...
  think:{ q:[`Does the page open with enough runway to build a wishlist before launch?`,`Which discount and price-change rules does this store enforce, and do our planned promotions fit them?`,`What does this store's revenue share actually pay us at our expected volume?`,`Who owns the tax interview and bank details, and when do they need to be filed?`],
  ...
  ai:{ yes:[`Draft store-page copy variants and capsule concepts for a human to choose between.`,`Summarize a store's asset specs and discount rules into a checklist.`,`Model revenue at several price points against a store's published share tiers.`],
  ...
  rel:[['ratings-and-disclosures',`The store page cannot go live until the rating and privacy answers behind it are filed.`],['launch-and-discoverability',`The store page is the asset discoverability work exists to fill.`],['business-model',`Regional pricing and each store's revenue share decide what the business model can actually fund.`],['release-and-updates',`Coming Soon and pre-orders run out at the release button this topic hands off to.`]] });
TECH('store-presence',[
...
INTERVIEW('store-presence',{
  ...
  mid:[
    ...
    { q:`You are choosing a launch price across three storefronts with different revenue shares. Walk through it.`,
      a:`Model expected revenue at each store's real share and payout timing, not a single headline number, and account for regional pricing tools that may already localize the price for you. Treat the shares as inputs to the price and platform mix, not an afterthought calculated post-launch.`,
  ...
  senior:[
    ...
    { q:`How do you decide whether a store's revenue share actually changes your pricing or platform strategy?`,
      ...
      follow:`The team wants to chase a full-revenue promotional window instead. What tradeoff are they making?`,
...
FACTS('store-presence',[
  ...
  { claim:`Apple's App Store commission is 30% standard, or 15% for qualifying subscriptions. Members of Apple's Small Business Program (no more than $1 million in proceeds in the prior calendar year, or new to the App Store; enrolment required) pay 15% on paid apps and in-app purchases.`, asOf:'2026-09-24', src:'https://developer.apple.com/app-store/small-business-program/' },
...
INTERVIEW('release-and-updates',{
  ...
  mid:[
    ...
    { q:`A balance patch changes how the save file is structured. What do you check before it ships?`,
      a:`That the save carries a version number the game checks on load, and that a migration path exists from every version still in the wild to the new one; then you actually load an old save through the new build before calling it done, rather than trusting the migration code by inspection alone.`,
