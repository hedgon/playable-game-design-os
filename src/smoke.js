...
const ROUTES_IN_PAGE = () => {
  ...
  return [
    ...
    '#/paths/' + p.id, '#/paths/' + p.id + '/' + st.id, '#/review', '#/sources', '#/games', '#/games/' + REFERENCE_GAMES[0].id, '#/platforms', '#/platforms/' + PLATFORMS[0].id, '#/platforms/' + PLATFORMS.find(x => x.kind === 'ugc').id, '#/engines', ...ENGINES.map(e => '#/engines/' + e.id), '#/guide',
