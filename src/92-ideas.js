...
function toolDissect(el){
  ...
  `<div class="grid c2"><div>
    ...
    ${field('ds_concept','Concept','Fantasy, core verb, player. Prefilled from the Idea Finder if you used it.', saved.concept, 2)}
    ...
    <div class="presets" id="ds_lib">${REFERENCE_GAMES.map(g => `<button data-g="${g.id}" class="${saved.comps.some(c=>c.id===g.id)?'on':''}">${esc(g.t)}</button>`).join('')}</div>
  ...
  const renderLib = () => $$('#ds_lib button').forEach(b => b.classList.toggle('on', saved.comps.some(c => c.id===b.dataset.g)));
  ...
  const out = () => { const a = saved.answers; const done = XREF_QS.filter(q => a[q.id] !== undefined).length; const score = XREF_QS.reduce((s,q) => s + (a[q.id] !== undefined ? q.opts[a[q.id]][1] : 0), 0);
    const matrix = saved.comps.length ? `<div class="tablewrap"><table class="small"><thead><tr><th></th><th>Want</th><th>Verb</th><th>Decision / minute</th></tr></thead><tbody><tr><td><b>Mine</b></td><td>${esc((idea.wish||idea.complaints||'').split(/[.\n]/)[0]||'?')}</td><td>${esc(idea.verb||'?')}</td><td>${esc(idea.loop_decision||'?')}</td></tr>${saved.comps.map(c => `<tr><td><b>${esc(c.t)}</b></td><td>${esc((c.want||'').slice(0,90))}</td><td>${esc((c.verb||'').slice(0,60))}</td><td>${esc((c.minute||'').slice(0,90))}</td></tr>`).join('')}</tbody></table></div>` : '';
    let verdict = '';
