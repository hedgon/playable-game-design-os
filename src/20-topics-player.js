...
T('who-is-the-player',{ d:'player', t:'Who is the player?', tag:'A concrete person with prior knowledge, a context, and a reason to be here. Not "gamers".',
  ...
  why:[`Every heuristic in design is relative to a player. "Too hard", "too slow", "confusing" mean nothing without a person.`,`Prior knowledge decides how much you must teach. A veteran of the genre reads conventions instantly. A newcomer reads nothing.`,`Context (commute, couch, competitive ladder) decides session length, input precision and tolerance for friction.`,`A game that targets everyone is optimized for no one and loses to games that made a choice.`],
  ...
  ai:{ yes:[`Draft candidate player sketches from your positioning and genre so you can react to them.`,`Enumerate genre conventions your player likely knows, with examples from named games.`,`Analyze reviews and forum discussions of comparable games to surface what those players praise and complain about.`,`Build a playtester screening questionnaire from the sketch.`],
       no:[`Decide who the player is. That is a strategic commitment, not an analysis.`,`Invent player behavior data. Ask it to mark inferences as inferences.`,`Replace observation: AI has never watched your player play your game.`] },
  ...
  verify:[`Are the "conventions this player knows" grounded in games they name, or generic genre lore?`,`Did the sketch smuggle in demographic labels instead of behaviors?`,`Which claims are inferred from data you provided and which are invented plausibility?`],
  test:[`Screen testers: do they match the sketch? Track whether matched and unmatched testers behave differently.`,`Do matched testers recognize the conventions you assumed without explanation?`,`Where do matched testers hesitate in the first five minutes? Those are conventions you assumed wrongly.`],
...
T('player-motivation',{ d:'player', t:'Player motivation', tag:'Competence, autonomy, relatedness: why people keep playing when nothing forces them to.',
  ...
  think:{ q:[`Which need does the core loop feed in the first ten minutes? Which does it feed at hour ten?`,`Where does the player feel they chose, versus were funneled?`,`Where does the player see themselves getting better, with evidence they can perceive?`,`Who does the player matter to inside or around the game?`],
  ...
  ai:{ yes:[`Map systems to motivational needs and flag gaps.`,`Generate alternative ways a given moment could feed competence, autonomy or relatedness.`,`Summarize the research on intrinsic versus extrinsic motivation as it applies to a specific mechanic you describe.`,`Analyze playtest transcripts for motivational language ("I figured out" versus "I got").`],
       no:[`Declare which need your game is about. That is identity.`,`Assert that a mechanic "is motivating" without behavioral evidence.`] },
  prompts:[{l:'Motivation audit',p:`Act as a skeptical player psychologist. Here is our core loop and major systems: [DESCRIPTION]. For each system, state which psychological need it primarily feeds (competence, autonomy, relatedness, curiosity) and how the player would perceive that. Then identify systems that feed no need, and moments where an extrinsic reward may be crowding out an intrinsic one. Propose observable playtest signals for each claim. Do not propose new features.`}],
  verify:[`Does the analysis cite observable player behavior, or does it label systems with psychology words?`,`Is "motivating" being asserted for the designer's intent rather than the player's experience?`],
...
TECH('player-motivation',[
  ...
  {n:'Gamer Motivation Profile', how:`Quantic Foundry's model scores twelve motivations in six pairs: Action (destruction, excitement), Social (competition, community), Mastery (challenge, strategy), Achievement (completion, power), Immersion (fantasy, story) and Creativity (design, discovery). Compare the profile of the audience you want with what your loop actually rewards.`, fit:`Positioning and audience fit: does the audience you want care about what your systems reward?`, cost:`Self-reported tendencies from surveys, not a measure of your game. A person's profile shifts with mood and company.`, alt:`Need mapping plus coded playtest language.`}
]);
...
T('fantasy',{ d:'player', t:'Fantasy and desire', tag:'What the player is buying is a version of themselves. Name it precisely.',
  what:`The fantasy is the identity, power or situation the player gets to inhabit: master thief, nurturing farmer, cunning general, unstoppable force, curious explorer. Desire is the pull toward it. The fantasy is the promise on the box. Every mechanic either pays it off or betrays it.`,
  ...
  how:[`Write the fantasy sentence and the three verbs it implies.`,`Check the core loop: are those verbs the ones the player actually performs most often? If not, either the loop or the fantasy is wrong.`,`List fantasy-breakers (moments that contradict the identity) and hunt them in playtests.`,`Use the fantasy as the first gate in feature evaluation.`],
  ai:{ yes:[`Generate many candidate fantasy statements from a rough premise so the human can recognize the right one.`,`Enumerate the verbs a fantasy implies and cross-check them against your mechanic list.`,`Hunt for fantasy-breakers in a design document or level script.`],
  ...
  verify:[`Did the AI restate the genre instead of the identity?`,`Are the "fantasy verbs" actually in the loop or only in the pitch?`],
