...
PATH('gameplay-engineer-unity', {
  ...
  stages:[
    { id:'s1', t:'The loop, in Unity', level:'beginner',
      ...
      check:{
        recall:[
          { q:'Why can reading a single-frame input inside FixedUpdate drop or double-count a press?', a:'FixedUpdate can run zero or twice within a single rendered frame, so a check for a press made there can miss it entirely or count it twice, producing exactly the input dropouts and double-triggers players report as an unresponsive control scheme.' },
    ...
    { id:'s4', t:'Choose and see, in Unity', level:'advanced',
      ...
      check:{
        recall:[
          { q:'How do you decide whether a decision is reactive or goal-directed?', a:'A reactive decision picks among options available right now and suits a state machine or behaviour tree. A goal-directed decision needs a sequence of actions toward a future state and suits a planner or utility layer. The agent\'s hardest decision decides which family fits.' },
          { q:'Why does storing a sighting in a knowledge component beat behaviour reading the player transform directly?', a:'A knowledge component stores only what the agent has actually sensed, so behaviour reading it stays fair and cannot see through walls or off screen. Reading the player\'s transform directly gives the agent information no sense channel produced, which players experience as cheating.' },
...
PATH('live-game-backend-engineer', {
  ...
  stages:[
    { id:'s1', t:'Shape the service', level:'intermediate',
      ...
      check:{
        recall:[
          { q:'Which direction should a dependency point, and what breaks when it points the other way?', a:'Dependencies should point inward, toward entities and use cases at the centre, with adapters like handlers and repositories outside. Pointed outward, the centre imports a database driver or transport directly, so a rule test needs real infrastructure and the game rules become tied to whatever stores them.' },
          { q:'What decides whether one binary can boot as more than one process role?', a:'A dependency graph generated at build time from provider sets lets main read a mode flag and call a different injector per role, so each role, API server, worker, or batch job, builds only the dependencies it needs from one shared set of entities and interactors.' },
    ...
    { id:'s2', t:'Handle the request', level:'intermediate',
      ...
      check:{
        recall:[
          ...
          { q:'What are the two disciplines that keep a transaction and a response both minimal?', a:'Scoping every transaction to the request, opened by the handler and committed or rolled back in one deferred place, and sending the client only the fields or tables a request actually changed rather than a full snapshot. Both keep a write or response from growing wider than the action that caused it.' },
          { q:'What must an AI output verification pass actually check, beyond whether the code compiles?', a:'Whether the code matches the actual design intent, not just that it builds: whether it introduced behaviour nobody asked for, whether error handling and transaction boundaries follow the agreed pattern, and whether it silently changed something the original brief never mentioned.' }
        ],
    ...
    { id:'s5', t:'Idioms and tests', level:'intermediate',
      ...
      check:{
        recall:[
          ...
          { q:'What does the AI Delegation Planner’s split actually decide?', a:'For each task it assigns an owner before work starts: Human, AI, both, or player evidence required for anything only a playtest can answer. It forces an explicit decision about who owns each task\'s judgment instead of defaulting to whoever is available.' }
        ],
...
PATH('netcode-server-engineer', {
  ...
  stages:[
    ...
    { id:'s2', t:'Sync and protocol', level:'advanced',
      ...
      check:{
        recall:[
          { q:'Why does sending what changed need a rate someone chose on purpose?', a:'Bandwidth and server cost scale with what you send times how often times how many players see it, so an undeliberate tick rate, picked because it matched the physics loop, becomes the largest running cost of the game, discovered during a postmortem instead of chosen on purpose.' },
          { q:'What is the minimum a message envelope needs, and what does adding a field beyond that cost you?', a:'A length, an op code, a message id, and a payload. Anything past that is a decision that has to be defended, because it grows every message and every parser reading it, and adding a field later means every existing client has to be updated too.' },
...
PATH('build-and-release-engineer', {
  ...
  stages:[
    ...
    { id:'s4', t:'Deploy and deliver content', level:'intermediate',
      ...
      check:{
        recall:[
          { q:'What property of a workload should actually decide its deploy model?', a:'Whether the workload holds state, how long it runs, and how often it is invoked. A stateless request should scale by adding copies, a long-lived connection needs a supervised or scheduled process, and a periodic job belongs on a clock, so the work\'s shape decides the model rather than fashion.' },
    ...
    { id:'s5', t:'Release on a rhythm', level:'intermediate',
      ...
      check:{
        recall:[
          ...
          { q:'Why does re-prefixing a failure with its phase name matter for a two-hour log?', a:'When every stage tags a failure with the name of the phase that produced it, a two-hour build log can be triaged by reading that single re-prefixed line instead of scrolling through the entire log to find where things actually went wrong.' },
          { q:'What does validating and restoring around every build actually prevent?', a:'Validating parameters in a cheap setup stage before an expensive build starts catches a bad input in the first minute instead of after hours of work, and restoring the working tree before each run stops a build that mutates or deletes files from leaving broken state for the next run.' }
        ],
...
PATH('interview-prep-engineer', {
  ...
  stages:[
    ...
    { id:'s2', t:'Infra, testing, and reliability', level:'intermediate',
      ...
      check:{
        recall:[
          ...
          { q:'Why is “a test that passed but should not have” a sharper story than “we have good test coverage”?', a:'We have good test coverage is unverifiable and often means little more than a high percentage. Describing a specific test whose expected value was pasted from the code\'s own output shows the candidate understands that a green suite can still prove nothing was actually checked.' },
          { q:'What would you actually say if asked how you decide what to delegate to AI?', a:'State the categories explicitly: this task is Human because it needs judgment, this one is AI because it is repetitive and checkable, this one is player evidence required because only a playtest can answer it. Naming the split for a real recent task beats a vague comfort level with tools.' }
        ],
    ...
    { id:'s3', t:'Game AI and production craft', level:'intermediate',
      ...
      check:{
        recall:[
          { q:'Why does naming the technique you did not pick make an answer stronger?', a:'It shows the candidate actually weighed the alternative\'s cost rather than reaching for the only technique they know. Explaining why a behaviour tree would have been harder to debug, or a planner too expensive to author, demonstrates the tradeoff was real, not assumed.' },
...
PATH('ship-it', {
  ...
  stages:[
    { id:'s1', t:'Pick your platforms', level:'intermediate',
      ...
      steps:[
        { kind:'topic', ref:'platform-choice', why:'The platform decides the engine, the input, the calendar and the audience before any feature does.', do:'Score two or three candidate platforms on reach, cost, gatekeeping and effort, and write which one your validated audience is actually on.', min:30 },
        ...
        { kind:'platform', ref:'steam', why:'Steam is the most open store, so it is the baseline every other gate is measured against.', do:'Read the Steam guide and list what you need on day one: the fee, the store page, both reviews and the Coming Soon runway.', min:25 },
    ...
    { id:'s3', t:'Pass the gates', level:'intermediate',
      ...
      steps:[
        { kind:'topic', ref:'certification-and-review', why:'Every platform has a gate and a clock, and a rejection costs the turnaround twice.', do:'Build a submission calendar backward from your launch date, with each platform’s review window and one resubmission of buffer.', min:30 },
    ...
    { id:'s4', t:'Launch and keep it running', level:'advanced',
      ...
      check:{
        recall:[
          ...
          { q:'How does release differ between Steam and the mobile stores?', a:'On Steam an approved game waits until you press Release. Apple and Google offer managed releases with phased or staged rollouts that can be paused.' }
        ],
