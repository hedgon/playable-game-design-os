...
| **Map** | The always-visible mind map, drawn as a horizontal tidy tree: the goal on the left, the fourteen domains on the next layer, and a domain's topics on the third layer while it is open. Nodes are labelled cards, so names never depend on hover. Click a domain to expand or collapse it in place, click a topic to read it in the right panel. Drag to pan, wheel or pinch to zoom, "fit" to reset. The tree stacks and pans as content grows instead of shrinking. Routes: `#/map/d/<domain>`, `#/map/t/<topic>`, `#/map/s/<smell>`, `#/map/home`. |
