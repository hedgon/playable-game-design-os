...
| X5 | SERIES `touhou` (ZUN, from 1996): the main shooting games, the fan-work guidelines that allow commercial fan games, and fan games that reached Steam and consoles; reception from sourced evidence only | landed |
| X6 | Image sources widened: official press screenshots, including those republished by gaming news sites, credited to the publisher via the outlet (licence `press`); the placeholder is removed; every series entry without a screen gets one (Final Fantasy XIV first, then the gaps in Megami Tensei and every series in progress) | shipped pages filled (Minecraft, Wordle, Animal Crossing, Tetris two lens shots each; Flappy Bird second shot; Final Fantasy XIV, four Megami Tensei entries, Mega Man 2); series in progress fill before landing |
| X7 | Image viewer: click or tap a screenshot, header or timeline image to open it large with its callouts; closes on the close button, a tap outside or Escape, one layer at a time; fits a phone screen | landed |
...
| G2.3+ | Proposed: Zero Escape: 999, Danganronpa, Doki Doki Literature Club | researching (999 fact-check, Danganronpa drafting) |
...
| G3.4+ | Proposed: Breath of the Wild, Elden Ring, The Witcher 3 (the Ubisoft tower formula as the compare counter-example, not an entry) | researching (Breath of the Wild covered by the Zelda series; Elden Ring drafting) |
| G4.1–G4.2 | Old-school fun: Dynasty Warriors (musou), Worms Armageddon | researching (both drafting; Dynasty Warriors as a series) |
